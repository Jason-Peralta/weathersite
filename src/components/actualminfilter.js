...
// actual_mean_temp
export default function ActualMinFilter(props) {
    const [selected, setSelected] = React.useState(null)
    const [value, setValue] = React.useState('')
    return (
        <Pane>
            <Pane display="flex" alignItems="center" marginX={majorScale(3)}>
                <Strong> Actual Min Temp </Strong>
            </Pane>
            <Pane display="flex" alignItems="center" padding={8} marginX={majorScale(3)}>
                <SelectMenu
                    ...
                    onSelect={
                        (item)=>{
                            ...
                            temp.actualMinTemp.RelationalModifier = item.value
                            props.handler(temp)
                        }
                    }
                >
                ...
                <TextInput
                    placeholder="Enter Number Of Actual Min Temp"
                    onChange={
                        (e)=>{
                            let temp = props.answers
                            temp.actualMinTemp.value = e.target.value
                            props.handler(temp)
                        }
                    }
                />
